...
public class cmd_world_channel extends PlayerCommand {
    ...
    public void execute(Player player, String... params) {
        int i = 1;
        int gp = CustomConfig.WORLD_CHANNEL_AP_COSTS;
        boolean check = true;
        String adminTag = "";
        ...
        if (player.isGM()) {
            ...
            if (player.getAbyssRank().getGp() < gp) {
                ...
                PacketSendUtility.sendMessage(player, LanguageHandler.translate(CustomMessageId.NOT_ENOUGH_AP) + player.getAbyssRank().getGp());
            } else {
                AbyssPointsService.addAp(player, -gp);
                World.getInstance().doOnAllPlayers(new Visitor<Player>() {
                    @Override
                    public void visit(Player player) {
                        PacketSendUtility.sendMessage(player, sMessage);
