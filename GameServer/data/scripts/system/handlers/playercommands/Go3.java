...
/**
 * @rework Imaginary, Uriel
 */
 
public class Go3 extends PlayerCommand {
	...
	public void execute(Player player, String... params) {
		...
		if (player.getPosition().getMapId() == WorldMapType.GAB1_SUB.getId()) {
			PacketSendUtility.sendMessage(player, "No puedes usar este comando en este mapa");
			return;
		}
		...
		if (player.getRace() == Race.ELYOS && !player.isInPrison()) {
			goTo(player, WorldMapType.TIAMARANTA_EYE_2.getId(), 750, 28, 1196);
		}
		else if (player.getRace() == Race.ASMODIANS && !player.isInPrison()) {
			goTo(player, WorldMapType.TIAMARANTA_EYE_2.getId(), 751, 1457, 1196);
		} else {
			return;
		}
	}
	...
	public void onFail(Player player, String message) {
		PacketSendUtility.sendMessage(player, "Syntax: .go3");
	}
}
