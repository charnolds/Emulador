...
public class MySQL5RewardServiceDAO extends RewardServiceDAO {
    ...
    public static final String UPDATE_QUERY = "UPDATE `web_reward` SET `rewarded`=?, received=NOW() WHERE `unique_id`=?";
    ...
    public FastList<RewardEntryItem> getAvailable(int playerId) {
        ...
        try {
            ...
            while (rset.next()) {
                int unique_id = rset.getInt("unique_id");
                int item_id = rset.getInt("item_id");
                long count = rset.getLong("item_count");
                list.add(new RewardEntryItem(unique_id, item_id, count));
            }
            rset.close();
            stmt.close();
        } catch (Exception e) {
