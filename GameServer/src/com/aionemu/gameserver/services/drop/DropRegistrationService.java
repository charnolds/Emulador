...
public class DropRegistrationService {
    ...
    private DropList dropList; // sql drop
    ...
    public void registerDrop(Npc npc, Player player, int heighestLevel, Collection<Player> groupMembers) {
        ...
        dropList = DropService.getInstance().getDropList(); // sql drop
        @SuppressWarnings("unused")
		VisibleObject visibleObject = player.getTarget(); // sql drop
        int npcObjId = npc.getObjectId();
        int npcTemplateId = npc.getObjectTemplate().getTemplateId(); // sql drop

        // Getting all possible drops for this Npc
        NpcDrop npcDrop = npc.getNpcDrop();
        Set<DropItem> droppedItems = new HashSet<DropItem>();
        Set<Drop> templates = dropList.getDropsFor(npcTemplateId); // sql drop
        int index = 1;
        int dropChance = 100;
        ...
        // sql drop
        if (templates != null) {
            for (Drop dropTemplate : templates) {
