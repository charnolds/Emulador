    ...
    public void verify(Player player) {
        ...
        for (RewardEntryItem item : list) {
            if (DataManager.ITEM_DATA.getItemTemplate(item.id) == null) {
                log.warn("[RewardController][" + item.unique_id + "] null template for item " + item.id + " on player " + player.getObjectId() + ".");
                continue;
            }
            ...
            try {
                ...
                log.info("[RewardController][" + item.unique_id + "] player " + player.getName() + " has received (" + item.count + ")" + item.id + ".");
                rewarded.add(item.unique_id);
            } catch (Exception e) {
                log.error("[RewardController][" + item.unique_id + "] failed to add item (" + item.count + ")" + item.id + " to " + player.getObjectId(), e);
                continue;
            }
        }
