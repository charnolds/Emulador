...
public class PlayerRestrictions extends AbstractRestrictions {
    ...
    public boolean canUseSkill(Player player, Skill skill) {
        ...
        if (skill.getSkillTemplate().getStack().contains("ABYSS_RANKERSKILL") && player.getWorldId() == 600100000) {
            PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_SKILL_CANT_CAST_IN_CURRENT_POSTION);
            return false;
        }
        if (skill.getSkillTemplate().getStack().contains("ABYSS_RANKERSKILL") && player.getWorldId() == 600070000) {
            PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_SKILL_CANT_CAST_IN_CURRENT_POSTION);
            return false;
        }
        if (skill.getSkillTemplate().getStack().contains("ABYSS_RANKERSKILL") && player.getWorldId() == 600090000) {
            PacketSendUtility.sendPacket(player, SM_SYSTEM_MESSAGE.STR_SKILL_CANT_CAST_IN_CURRENT_POSTION);
            return false;
        }
